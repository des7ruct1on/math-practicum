...
int main(int argc, char* argv[]) {
    ...
    vector4.coordinates[0] = 1.0;
    vector4.coordinates[1] = 2.0;
    vector4.coordinates[2] = -4.0;

    vector5.coordinates = (double*)malloc(dimension * sizeof(double));
    vector5.coordinates[0] = -1.0;
    vector5.coordinates[1] = 2.0;
    vector5.coordinates[2] = 4.0;
    int count = 5;
    int size;
    int p = 1;
    status_code st_find = get_max_len_vector(&maxs, &size, get_golder_norm, dimension, p, count, vector1, vector2, vector3, vector4, vector5);
