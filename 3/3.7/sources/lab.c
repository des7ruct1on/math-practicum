...
void free_liver(Liver* tmp) {
    ...
    if (tmp->surname) string_clear(tmp->surname); free(tmp->surname);
    if (tmp->name) string_clear(tmp->name); free(tmp->name);
    if (tmp->last_name) string_clear(tmp->last_name); free(tmp->last_name);
    if (tmp->date_birth) string_clear(tmp->date_birth); free(tmp->date_birth);
    if (tmp->gender) string_clear(tmp->gender); free(tmp->gender);
    free(tmp);
}
...
int compare_citizen(Liver* cur, char* _name, char* _surname, char* _last_name) {
    if (!cur) return 1;
    if (!strcmp(_last_name, "")) return strcmp(cur->name->data, _name) + strcmp(cur->surname->data, _surname);
    return strcmp(cur->name->data, _name) + strcmp(cur->surname->data, _surname) + strcmp(cur->last_name->data, _last_name);
}
...
status_code remove_list(List_node* list, My_string* key, Stack* stack) {
    ...
    while (cur->next->data != NULL) {
        int check = compare_citizen(cur->next->data, name, surname, last_name);
        if (check == 0) {
            break;
        }
        _index++;
...
status_code undo(List_node* list, Stack* stack) {
    ...
    for (int i = 0; i < size / 2; i++) {
        ...
        if (was == cmd_add) {
            concat_strings(&info, cur->surname);
            concat_strings(&info, sep);
            concat_strings(&info, cur->name);
            if (cur->last_name->size != 0) {
                concat_strings(&info, sep);
                concat_strings(&info, cur->last_name);
            }
            ...
            st_act = push_list(&list, now->condition, tmp);
            if (st_act != code_success) {
                stack_destroy(tmp);
                string_clear(info);
                free(info);
